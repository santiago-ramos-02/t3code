...
Sign in to T3 Connect, link your environments, and enable **Device Notifications** in **Settings → Notifications** to receive alerts when an agent finishes, fails, needs approval, or asks for input. An agent that hands work to background subagents finishes when they do, so each handoff does not alert. Tap a notification to open its thread. Your environment must have agent activity publishing enabled.
