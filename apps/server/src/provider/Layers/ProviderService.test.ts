...
function makeCustomProviderServiceLayer(
  registry: ProviderAdapterRegistry.ProviderAdapterRegistry["Service"],
  input: {
    readonly directory?: ProviderSessionDirectory.ProviderSessionDirectory["Service"];
    readonly issueMcpCredential?: ProviderServiceLiveOptions["issueMcpCredential"];
  } = {},
) {
  ...
  return Layer.mergeAll(
    makeProviderServiceLive(
      input.issueMcpCredential === undefined
        ? undefined
        : { issueMcpCredential: input.issueMcpCredential },
    ).pipe(
      Layer.provide(NodeServices.layer),
...
it.effect(
  "routes approval and user input through a provisional startup binding, then persists success",
  () =>
    Effect.gen(function* () {
      const base = makeFakeCodexAdapter();
      const startupReady = yield* Deferred.make<void>();
      const approvalResponse = yield* Deferred.make<void>();
      const userInputReady = yield* Deferred.make<void>();
      const userInputResponse = yield* Deferred.make<void>();
      const startSession: ProviderAdapterShape<ProviderAdapterError>["startSession"] = vi.fn(
        (input) =>
          Effect.gen(function* () {
            const session = yield* base.startSession(input);
            yield* Deferred.succeed(startupReady, undefined);
            yield* Deferred.await(approvalResponse);
            yield* Deferred.succeed(userInputReady, undefined);
            yield* Deferred.await(userInputResponse);
            return session;
          }),
      );
      const respondToRequest = vi.fn(
        (
          ...
          Deferred.succeed(approvalResponse, undefined).pipe(Effect.asVoid),
      );
      const respondToUserInput = vi.fn(
        (
          _threadId: ThreadId,
          ...
          Deferred.succeed(userInputResponse, undefined).pipe(Effect.asVoid),
      );
      const adapter = {
        ...base.adapter,
        startSession,
      ...
      const layer = makeCustomProviderServiceLayer(
        makeStaticInstanceRegistry([[codexInstanceId, adapter]]),
      );

      yield* Effect.gen(function* () {
        const provider = yield* ProviderService.ProviderService;
        const directory = yield* ProviderSessionDirectory.ProviderSessionDirectory;
        const threadId = asThreadId("thread-provisional-startup-routing");
        ...
        const startFiber = yield* provider
          .startSession(threadId, {
            ...
            providerInstanceId: codexInstanceId,
            threadId,
            cwd: fixtureCwd("provisional-startup-routing"),
            runtimeMode: "full-access",
          })
          .pipe(
            Effect.tap(() => Effect.sync(() => (startResolved = true))),
            Effect.forkChild,
          );
        yield* Deferred.await(startupReady);

        const beforeSuccess = yield* directory.getBinding(threadId);
        const approval = yield* provider
          .respondToRequest({
            threadId,
          ...
          .pipe(Effect.exit);
        if (Exit.isFailure(approval)) yield* Deferred.succeed(approvalResponse, undefined);
        yield* Deferred.await(userInputReady);
        const userInput = yield* provider
          .respondToUserInput({
            threadId,
            requestId: asRequestId("startup-input"),
            answers: { startup_input: "ready" },
          })
          .pipe(Effect.exit);
        if (Exit.isFailure(userInput)) yield* Deferred.succeed(userInputResponse, undefined);
        const session = yield* Fiber.join(startFiber);
        const finalized = yield* directory.getBinding(threadId);

        assert.equal(startResolved, true);
        assert(Exit.isSuccess(approval));
        assert(Exit.isSuccess(userInput));
        assert(Option.isNone(beforeSuccess));
        assert.deepEqual(respondToRequest.mock.calls, [
          [threadId, asRequestId("startup-approval"), "accept"],
        ]);
        assert.deepEqual(respondToUserInput.mock.calls, [
          [threadId, asRequestId("startup-input"), { startup_input: "ready" }],
        ]);
        assert.equal(session.providerInstanceId, codexInstanceId);
        assert(Option.isSome(finalized));
        assert.equal(finalized.value.providerInstanceId, codexInstanceId);
      }).pipe(Effect.provide(layer));
    }),
);

it.effect("removes a new-thread provisional binding when startup fails", () =>
  Effect.gen(function* () {
    const base = makeFakeCodexAdapter();
    const startupReady = yield* Deferred.make<void>();
    const failStartup = yield* Deferred.make<void>();
    const startSession: ProviderAdapterShape<ProviderAdapterError>["startSession"] = vi.fn(
      (input) =>
        Effect.gen(function* () {
          yield* base.startSession(input);
          yield* Deferred.succeed(startupReady, undefined);
          yield* Deferred.await(failStartup);
    ...
    yield* Effect.gen(function* () {
      const provider = yield* ProviderService.ProviderService;
      const directory = yield* ProviderSessionDirectory.ProviderSessionDirectory;
      const threadId = asThreadId("thread-provisional-startup-failure");
      const startFiber = yield* provider
        .startSession(threadId, {
          provider: CODEX_DRIVER,
          providerInstanceId: codexInstanceId,
          threadId,
          cwd: fixtureCwd("provisional-startup-failure"),
          runtimeMode: "full-access",
        })
        .pipe(Effect.exit, Effect.forkChild);
      yield* Deferred.await(startupReady);
      assert(Option.isNone(yield* directory.getBinding(threadId)));

      yield* Deferred.succeed(failStartup, undefined);
      const outcome = yield* Fiber.join(startFiber);
      const response = yield* provider
        .respondToRequest({
          threadId,
          requestId: asRequestId("after-failed-start"),
          decision: "accept",
        })
        .pipe(Effect.exit);
      ...
      assert(Option.isNone(yield* directory.getBinding(threadId)));
      assert(Exit.isFailure(response));
      assert.equal(base.respondToRequest.mock.calls.length, 0);
    }).pipe(Effect.provide(layer));
  }),
);

it.effect(
  "routes a provider-switch startup provisionally and restores the old binding on failure",
  () =>
    Effect.gen(function* () {
      const previous = makeFakeCodexAdapter(CODEX_DRIVER);
      const nextBase = makeFakeCodexAdapter(CLAUDE_AGENT_DRIVER);
      const nextInstanceId = ProviderInstanceId.make("claude-switch-target");
      const startupReady = yield* Deferred.make<void>();
      const failStartup = yield* Deferred.make<void>();
      const nextStartSession: ProviderAdapterShape<ProviderAdapterError>["startSession"] = vi.fn(
        (input) =>
          Effect.gen(function* () {
            yield* nextBase.startSession(input);
            yield* Deferred.succeed(startupReady, undefined);
            yield* Deferred.await(failStartup);
            return yield* new ProviderAdapterRequestError({
              provider: CLAUDE_AGENT_DRIVER,
      ...
      const nextAdapter = {
        ...nextBase.adapter,
        startSession: nextStartSession,
      } satisfies ProviderAdapterShape<ProviderAdapterError>;
      const layer = makeCustomProviderServiceLayer(
        makeStaticInstanceRegistry([
          [codexInstanceId, previous.adapter],
          [nextInstanceId, nextAdapter],
        ]),
      );
      ...
      yield* Effect.gen(function* () {
        const provider = yield* ProviderService.ProviderService;
        const directory = yield* ProviderSessionDirectory.ProviderSessionDirectory;
        const threadId = asThreadId("thread-provisional-provider-switch");
        yield* provider.startSession(threadId, {
          provider: CODEX_DRIVER,
          providerInstanceId: codexInstanceId,
          threadId,
          cwd: fixtureCwd("provisional-provider-switch"),
          runtimeMode: "full-access",
        });
        previous.respondToRequest.mockClear();

        const switchFiber = yield* provider
          .startSession(threadId, {
            provider: CLAUDE_AGENT_DRIVER,
          ...
          .pipe(Effect.exit, Effect.forkChild);
        yield* Deferred.await(startupReady);
        assert.deepEqual(previous.stopSession.mock.calls, [[threadId]]);
        assert.equal(yield* previous.hasSession(threadId), false);
        yield* provider.respondToRequest({
          threadId,
          requestId: asRequestId("switch-startup-request"),
          decision: "accept",
        });

        assert.equal(previous.respondToRequest.mock.calls.length, 0);
        assert.deepEqual(nextBase.respondToRequest.mock.calls, [
          [threadId, asRequestId("switch-startup-request"), "accept"],
        ]);
        yield* Deferred.succeed(failStartup, undefined);
        assert(Exit.isFailure(yield* Fiber.join(switchFiber)));

        const restored = yield* directory.getBinding(threadId);
        assert(Option.isSome(restored));
        assert.equal(restored.value.providerInstanceId, codexInstanceId);
        previous.startSession.mockClear();
        yield* provider.respondToRequest({
          threadId,
          requestId: asRequestId("restored-request"),
          decision: "decline",
        });
        assert.equal(previous.startSession.mock.calls.length, 1);
        assert.deepEqual(previous.respondToRequest.mock.calls, [
          [threadId, asRequestId("restored-request"), "decline"],
        ]);
      }).pipe(Effect.provide(layer));
    }),
);

it.effect("repairs a stopped non-resumable binding when a provider switch fails", () =>
  Effect.gen(function* () {
    const previousBase = makeFakeCodexAdapter(CODEX_DRIVER);
    const previousStartSession = vi.fn((input: ProviderSessionStartInput) =>
      previousBase.startSession(input).pipe(
        Effect.map((session) => {
          const { resumeCursor: _resumeCursor, ...sessionWithoutResumeCursor } = session;
          previousBase.updateSession(input.threadId, () => sessionWithoutResumeCursor);
          return sessionWithoutResumeCursor;
        }),
      ),
    );
    const previousAdapter = {
      ...previousBase.adapter,
      startSession: previousStartSession,
    } satisfies ProviderAdapterShape<ProviderAdapterError>;
    const nextBase = makeFakeCodexAdapter(CLAUDE_AGENT_DRIVER);
    const nextInstanceId = ProviderInstanceId.make("claude-non-resumable-switch-target");
    const startupReady = yield* Deferred.make<void>();
    const failStartup = yield* Deferred.make<void>();
    const nextStartSession: ProviderAdapterShape<ProviderAdapterError>["startSession"] = vi.fn(
      (input) =>
    ...
    const layer = makeCustomProviderServiceLayer(
      makeStaticInstanceRegistry([
        [codexInstanceId, previousAdapter],
        [nextInstanceId, nextAdapter],
      ]),
    );
    ...
    yield* Effect.gen(function* () {
      const provider = yield* ProviderService.ProviderService;
      const directory = yield* ProviderSessionDirectory.ProviderSessionDirectory;
      const threadId = asThreadId("thread-non-resumable-provider-switch");
      const cwd = fixtureCwd("non-resumable-provider-switch");
      yield* provider.startSession(threadId, {
        provider: CODEX_DRIVER,
        providerInstanceId: codexInstanceId,
        threadId,
      ...
      const switchFiber = yield* provider
        ...
        .pipe(Effect.exit, Effect.forkChild);
      yield* Deferred.await(startupReady);
      assert.deepEqual(previousBase.stopSession.mock.calls, [[threadId]]);
      assert.equal(yield* previousBase.hasSession(threadId), false);

      yield* Deferred.succeed(failStartup, undefined);
      assert(Exit.isFailure(yield* Fiber.join(switchFiber)));

      const repaired = yield* directory.getBinding(threadId);
      assert(Option.isSome(repaired));
      assert.equal(repaired.value.providerInstanceId, codexInstanceId);
      assert.equal(repaired.value.status, "stopped");
      assert.equal(repaired.value.resumeCursor == null, true);
      const staleRoute = yield* provider
        .respondToRequest({
          threadId,
          requestId: asRequestId("non-resumable-stale-request"),
          decision: "decline",
        })
        .pipe(Effect.exit);
      assert(Exit.isFailure(staleRoute));
      assert.equal(previousStartSession.mock.calls.length, 1);
      ...
      assert.equal(yield* previousBase.hasSession(threadId), true);
      assert.equal(previousStartSession.mock.calls.length, 2);
      assert.deepEqual(nextBase.stopSession.mock.calls, [[threadId]]);
    }).pipe(Effect.provide(layer));
  }),
);

it.effect(
  "keeps newer MCP and session ownership when stale startup succeeds after supersession",
  () =>
    Effect.gen(function* () {
      const previous = makeFakeCodexAdapter(CODEX_DRIVER);
      const staleBase = makeFakeCodexAdapter(CLAUDE_AGENT_DRIVER);
      const latestBase = makeFakeCodexAdapter(CURSOR_DRIVER);
      const staleInstanceId = ProviderInstanceId.make("stale-startup");
      const latestInstanceId = ProviderInstanceId.make("latest-startup");
      const staleIssueStarted = yield* Deferred.make<void>();
      const releaseStaleIssue = yield* Deferred.make<void>();
      let latestIssueCompleted = false;
      const staleReady = yield* Deferred.make<void>();
      const finishStale = yield* Deferred.make<void>();
      const latestReady = yield* Deferred.make<void>();
      const finishLatest = yield* Deferred.make<void>();
      const staleStart: ProviderAdapterShape<ProviderAdapterError>["startSession"] = vi.fn(
        (input) =>
          Effect.gen(function* () {
            const session = yield* staleBase.startSession(input);
            yield* Deferred.succeed(staleReady, undefined);
            yield* Deferred.await(finishStale);
            return session;
          }),
      );
      const latestStart: ProviderAdapterShape<ProviderAdapterError>["startSession"] = vi.fn(
        (input) =>
          Effect.gen(function* () {
            const session = yield* latestBase.startSession(input);
            yield* Deferred.succeed(latestReady, undefined);
            yield* Deferred.await(finishLatest);
            return session;
          }),
      );
      const staleAdapter = {
        ...staleBase.adapter,
        startSession: staleStart,
      } satisfies ProviderAdapterShape<ProviderAdapterError>;
      const latestAdapter = {
        ...latestBase.adapter,
        startSession: latestStart,
      } satisfies ProviderAdapterShape<ProviderAdapterError>;
      const issueMcpCredential: NonNullable<ProviderServiceLiveOptions["issueMcpCredential"]> = (
        request,
      ) =>
        Effect.gen(function* () {
          if (request.providerInstanceId === staleInstanceId) {
            yield* Deferred.succeed(staleIssueStarted, undefined);
            yield* Deferred.await(releaseStaleIssue);
          } else if (request.providerInstanceId === latestInstanceId) {
            latestIssueCompleted = true;
          }
          return {
            config: {
              environmentId: EnvironmentId.make("environment-startup-ownership"),
              threadId: request.threadId,
              providerSessionId: `mcp-${request.providerInstanceId}`,
              providerInstanceId: request.providerInstanceId,
              endpoint: "http://127.0.0.1/mcp",
              authorizationHeader: `Bearer ${request.providerInstanceId}`,
      ...
      const layer = makeCustomProviderServiceLayer(
        makeStaticInstanceRegistry([
          [codexInstanceId, previous.adapter],
          [staleInstanceId, staleAdapter],
          [latestInstanceId, latestAdapter],
        ]),
        { issueMcpCredential },
      );
      ...
      yield* Effect.gen(function* () {
        const provider = yield* ProviderService.ProviderService;
        const directory = yield* ProviderSessionDirectory.ProviderSessionDirectory;
        const threadId = asThreadId("thread-stale-provisional-cleanup");
        const cwd = fixtureCwd("stale-provisional-cleanup");
        yield* provider.startSession(threadId, {
          provider: CODEX_DRIVER,
          providerInstanceId: codexInstanceId,
          threadId,
        ...
        const staleFiber = yield* provider
          .startSession(threadId, {
            provider: CLAUDE_AGENT_DRIVER,
          ...
          .pipe(Effect.exit, Effect.forkChild);
        yield* Deferred.await(staleIssueStarted);
        const latestFiber = yield* provider
          .startSession(threadId, {
            provider: CURSOR_DRIVER,
            providerInstanceId: latestInstanceId,
            threadId,
            cwd,
            runtimeMode: "full-access",
          })
          .pipe(Effect.forkChild);
        for (let attempt = 0; attempt < 20 && !latestIssueCompleted; attempt += 1) {
          yield* Effect.yieldNow;
        }
        assert.equal(latestIssueCompleted, false);
        yield* Deferred.succeed(releaseStaleIssue, undefined);
        yield* Deferred.await(staleReady);
        yield* Deferred.await(latestReady);

        assert.equal(latestIssueCompleted, true);
        assert.equal(
          McpProviderSession.readMcpProviderSession(threadId)?.providerInstanceId,
          latestInstanceId,
        );
        yield* Deferred.succeed(finishStale, undefined);
        assert(Exit.isFailure(yield* Fiber.join(staleFiber)));
        assert.deepEqual(staleBase.stopSession.mock.calls, [[threadId]]);
        assert.equal(yield* staleBase.hasSession(threadId), false);
        assert.equal(
          McpProviderSession.readMcpProviderSession(threadId)?.providerInstanceId,
          latestInstanceId,
        );
        yield* provider.respondToRequest({
          threadId,
          requestId: asRequestId("latest-provisional-request"),
          decision: "accept",
        });
        assert.equal(previous.respondToRequest.mock.calls.length, 0);
        assert.equal(staleBase.respondToRequest.mock.calls.length, 0);
        assert.deepEqual(latestBase.respondToRequest.mock.calls, [
          [threadId, asRequestId("latest-provisional-request"), "accept"],
        ]);

        yield* Deferred.succeed(finishLatest, undefined);
        yield* Fiber.join(latestFiber);
        const finalized = yield* directory.getBinding(threadId);
        assert(Option.isSome(finalized));
        assert.equal(finalized.value.providerInstanceId, latestInstanceId);
      }).pipe(Effect.provide(layer));
    }),
);

it.effect("stops an attempt session when startup is interrupted after the adapter creates it", () =>
  Effect.gen(function* () {
    const base = makeFakeCodexAdapter();
    ...
    yield* Effect.gen(function* () {
      const provider = yield* ProviderService.ProviderService;
      const directory = yield* ProviderSessionDirectory.ProviderSessionDirectory;
      const threadId = asThreadId("thread-interrupted-startup-owner");
      const startFiber = yield* provider
        .startSession(threadId, {
          provider: CODEX_DRIVER,
          providerInstanceId: codexInstanceId,
          threadId,
          cwd: fixtureCwd("interrupted-startup-owner"),
          runtimeMode: "full-access",
        })
        .pipe(Effect.forkChild);
      ...
      assert.deepEqual(base.stopSession.mock.calls, [[threadId]]);
      assert.equal(yield* base.hasSession(threadId), false);
      assert(Option.isNone(yield* directory.getBinding(threadId)));
    }).pipe(Effect.provide(layer));
  }),
);

it.effect("stops an attempt session when durable startup finalization fails", () =>
  Effect.gen(function* () {
    const base = makeFakeCodexAdapter();
    const threadId = asThreadId("thread-startup-persistence-failure");
    ...
    const directory: ProviderSessionDirectory.ProviderSessionDirectory["Service"] = {
      upsert: (binding) =>
        binding.threadId === threadId
          ? Effect.die("durable upsert failed")
          : Effect.sync(() => {
              bindings.set(binding.threadId, binding);
            }),
      getBinding: (requestedThreadId) =>
        Effect.succeed(Option.fromNullishOr(bindings.get(requestedThreadId))),
      getProvider: (requestedThreadId) => {
        const binding = bindings.get(requestedThreadId);
        return binding === undefined
          ? Effect.die("binding missing")
    ...
    const layer = makeCustomProviderServiceLayer(
      makeStaticInstanceRegistry([[codexInstanceId, base.adapter]]),
      { directory },
    );
    ...
    yield* Effect.gen(function* () {
      ...
      const outcome = yield* provider
        .startSession(threadId, {
          ...
          providerInstanceId: codexInstanceId,
          threadId,
          cwd: fixtureCwd("startup-persistence-failure"),
          runtimeMode: "full-access",
        })
        .pipe(Effect.exit);
      ...
      assert.deepEqual(base.stopSession.mock.calls, [[threadId]]);
      assert.equal(yield* base.hasSession(threadId), false);
      assert.equal(bindings.has(threadId), false);
    }).pipe(Effect.provide(layer));
  }),
);

it.effect(
  "keeps committed startup state consistent when interrupted after durable upsert writes",
  () =>
    Effect.gen(function* () {
      const base = makeFakeCodexAdapter();
      const threadId = asThreadId("thread-startup-commit-boundary");
      const upsertWritten = yield* Deferred.make<void>();
        ...
        upsert: (binding) =>
          Effect.gen(function* () {
            ...
            if (binding.threadId === threadId) {
              yield* Deferred.succeed(upsertWritten, undefined);
      ...
      const issueMcpCredential: NonNullable<ProviderServiceLiveOptions["issueMcpCredential"]> = (
        ...
        Effect.succeed({
          config: {
            environmentId: EnvironmentId.make("environment-startup-commit-boundary"),
            threadId: request.threadId,
            providerSessionId: "mcp-startup-commit-boundary",
            providerInstanceId: request.providerInstanceId,
            endpoint: "http://127.0.0.1/mcp",
            authorizationHeader: "Bearer startup-commit-boundary",
            capabilities: request.capabilities,
          },
        });
      const layer = makeCustomProviderServiceLayer(
        makeStaticInstanceRegistry([[codexInstanceId, base.adapter]]),
        { directory, issueMcpCredential },
      );
      ...
      yield* Effect.gen(function* () {
        ...
        const startFiber = yield* provider
          .startSession(threadId, {
            ...
            providerInstanceId: codexInstanceId,
            threadId,
            cwd: fixtureCwd("startup-commit-boundary"),
            runtimeMode: "full-access",
          })
          .pipe(Effect.forkChild);
        yield* Deferred.await(upsertWritten);
        ...
        const outcome = yield* Fiber.await(startFiber);

        assert(Exit.isFailure(outcome));
        assert.equal(bindings.get(threadId)?.providerInstanceId, codexInstanceId);
        assert.equal(yield* base.hasSession(threadId), true);
        assert.equal(base.stopSession.mock.calls.length, 0);
        assert.equal(
          McpProviderSession.readMcpProviderSession(threadId)?.providerInstanceId,
          codexInstanceId,
        );
